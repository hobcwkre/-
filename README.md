...
## 權證 Black-Scholes 評價（頁⑥，獨立於回測流程）

與蒙地卡羅回測流程分開的獨立分頁，只需輸入權證代號即可：

- **理論價**：用標的股歷史波動率（HV，可調視窗天數）代入 Black-Scholes 計算，與市價比較→
  價格面偏貴／偏便宜（`mispricing_pct`）。
- **隱含波動率（IV）**：反推自市價本身（Newton-Raphson，失敗時退回二分法），與 HV 比較→
  波動率面偏貴／偏便宜。刻意不讓 IV 拿去算理論價（那樣理論價恆等於市價，比較會失去意義）。
- Greeks 同時輸出「每股標準值」與「每張權證實際值」（× 履約比例）兩種，避免混淆。
- 美式權證以歐式模型近似（無股利下提前履約不划算，業界常見簡化）。
- 資料源：`tpex_warrant_issue`（履約價／到期日／履約比例／買賣權／歐美式），首次查詢時
  整批（~9k 檔）懶載入進 `warrant_terms` 表；`ListedDate`/`ExpiryDate` 是西元 8 碼，
  與同回應中 `Date` 欄位的民國 7 碼格式不同，轉換時要分開處理。

## 蒙地卡羅回測分析網頁（React + FastAPI）

**標的範圍：上櫃（股票／ETF／權證）＋自訂上傳資料。**

- 權證（約 9,000 檔）不做全量入庫——回測到哪一檔才透過
  `afterTrading/tradingStock`（一次一個月）按需抓取並快取
  （`fetched_months` 表記錄已抓月份，避免對無成交月份重複請求）。
- 自訂資料：頁① 可上傳 CSV（欄位支援 date/日期＋close/收盤價，
  西元或民國年、UTF-8 或 Big5），上傳後出現在「自訂資料」類別，
  可與其他標的混搭回測；同名資料集會被覆蓋。
- 頁⑤ 報表對照 TEJ Pro 選股回測手冊：無風險利率設定、
  算術／幾何平均報酬、偏態、峰態、相關係數、Alpha/Beta、
  年化與下跌標準差、夏普／索提諾／崔諾／信息指標、勝率與期望值、
  各年度報酬、回撤明細（起始日／谷底日／虧損回復日／落水期間）。
